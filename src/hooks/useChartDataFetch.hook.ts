  ...
  const requestIntervalFn = () => {
    ...
    try {
      ...
      if (requestUrl?.value) {
        ...
        let object1 = Object.assign({}, targetComponent.request);
          if(object1.requestParams.Params.hasOwnProperty("start")) {
              // 获取当前时间
              const now = new Date();
              // 计算3天前的时间
              const threeDaysAgo = new Date(now);
              threeDaysAgo.setDate(threeDaysAgo.getDate() - 1);
              if (object1.requestParams.Params) {
                  let object = object1.requestParams.Params
                  let key: keyof typeof object
                  for (key in object) {
                      if (key === "start") {
                          object[key] = threeDaysAgo.toISOString()
                      } else if (key === "end") {
                          object[key] = now.toISOString()
                      }
                  }
              }
          }
          if(JSON.parse(object1.requestParams.Body.json).hasOwnProperty("start")) {
              // 获取当前时间
              const now = new Date();
              // 计算3天前的时间
              const threeDaysAgo = new Date(now);
              threeDaysAgo.setDate(threeDaysAgo.getDate() - 1);
           if(object1.requestParams.Body.json) {
               let object = JSON.parse(object1.requestParams.Body.json)
               let key: keyof typeof object
               for (key in object) {
                   if (key === "start") {
                       object[key] = threeDaysAgo.toISOString()
               ...
               object1.requestParams.Body.json = JSON.stringify(object)
           }
       }
        ...
        const fetchFn = async () => {
          const res = await customizeHttp(toRaw(object1), toRaw(chartEditStore.getRequestGlobalConfig))
          if (res) {
            try {
